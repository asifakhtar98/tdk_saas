...
class SignInBloc extends Bloc<SignInEvent, SignInState> {
  ...
  final VerifyMagicLinkUseCase _verifyMagicLinkUseCase;
  ...
  Future<void> _onSignUpRequested(
    ...
    final result = await _signUpWithEmailUseCase(
      SignUpWithEmailParams(email: event.email),
    );

    result.fold(
      (failure) => emit(SignInState.failure(failure)),
      (_) => emit(SignInState.magicLinkSent(email: event.email)),
    );
  }
  ...
  Future<void> _onVerificationRequested(
    MagicLinkVerificationRequested event,
    Emitter<SignInState> emit,
  ) async {
    emit(const SignInState.verificationInProgress());
    final result = await _verifyMagicLinkUseCase(
      VerifyMagicLinkParams(email: event.email, token: event.token),
    );
