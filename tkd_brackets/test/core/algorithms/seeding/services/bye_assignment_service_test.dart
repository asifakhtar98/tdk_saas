...
void main() {
  ...
  ByeAssignmentResult extractResult(
    Either<Failure, ByeAssignmentResult> result,
  ...
  group('ByeAssignmentService', () {
    ...
    group('zero byes (power of 2)', () {
      test('2 participants → 0 byes, bracketSize 2', () {
        final r = extractResult(
          service.assignByes(const ByeAssignmentParams(participantCount: 2)),
        );
      ...
      test('8 participants → 0 byes, bracketSize 8', () {
        final r = extractResult(
          service.assignByes(const ByeAssignmentParams(participantCount: 8)),
        );
        expect(r.byeCount, equals(0));
    ...
    group('bye distribution', () {
      test('3 participants → 1 bye, seed 1 gets bye', () {
        final r = extractResult(
          service.assignByes(const ByeAssignmentParams(participantCount: 3)),
        );
      ...
      test('5 participants → 3 byes distributed across halves', () {
        final r = extractResult(
          service.assignByes(const ByeAssignmentParams(participantCount: 5)),
        );
        ...
        expect(
          bottomHalf,
          greaterThan(0),
          reason: 'At least 1 bye in bottom half',
        );
      });

      test('7 participants → 1 bye', () {
        final r = extractResult(
          service.assignByes(const ByeAssignmentParams(participantCount: 7)),
        );
        expect(r.byeCount, equals(1));
        expect(r.bracketSize, equals(8));
        expect(r.byePlacements[0].seedPosition, equals(1));
      ...
      test('6 participants → 2 byes, bracketSize 8', () {
        final r = extractResult(
          service.assignByes(const ByeAssignmentParams(participantCount: 6)),
        );
      ...
      test('9 participants → 7 byes, bracketSize 16', () {
        final r = extractResult(
          service.assignByes(const ByeAssignmentParams(participantCount: 9)),
        );
      ...
      test('15 participants → 1 bye, bracketSize 16', () {
        final r = extractResult(
          service.assignByes(const ByeAssignmentParams(participantCount: 15)),
        );
        expect(r.byeCount, equals(1));
        expect(r.bracketSize, equals(16));
        expect(r.totalRounds, equals(4));
      ...
      test('17 participants → 15 byes, bracketSize 32', () {
        final r = extractResult(
          service.assignByes(const ByeAssignmentParams(participantCount: 17)),
        );
      ...
      test('33 participants → 31 byes, bracketSize 64', () {
        final r = extractResult(
          service.assignByes(const ByeAssignmentParams(participantCount: 33)),
        );
      ...
      test('all bye seeds are top seeds (1, 2, 3, ...)', () {
        ...
        final seeds = r.byePlacements.map((p) => p.seedPosition).toList()
          ..sort();
    ...
    group('invariants for all sizes', () {
      test('bye slots and participant slots never overlap', () {
        for (final n in [3, 5, 6, 7, 9, 15, 17, 33]) {
          final r = extractResult(
            service.assignByes(ByeAssignmentParams(participantCount: n)),
          );
          for (final p in r.byePlacements) {
            expect(
              p.bracketSlot,
              isNot(equals(p.byeSlot)),
              reason: 'n=$n: participant slot and bye slot must differ',
            );
            expect(
              r.byeSlots.contains(p.byeSlot),
              isTrue,
              reason: 'n=$n: byeSlot must be in byeSlots set',
            );
            expect(
              r.byeSlots.contains(p.bracketSlot),
              isFalse,
          ...
          expect(
            r.byeCount,
            equals(r.byePlacements.length),
            reason: 'n=$n: byeCount must equal byePlacements length',
          );
          expect(
            r.byeCount,
            equals(r.byeSlots.length),
