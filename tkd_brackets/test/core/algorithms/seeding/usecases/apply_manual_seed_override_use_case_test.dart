      ...
      test('pin position out of range returns ValidationFailure', () async {
        final result = await useCase(
          ApplyManualSeedOverrideParams(
            ...
            pinnedSeeds: const {
              '1': 3,
            }, // Only 2 participants, bracketSize should be 2.
          ),
        );
    ...
    group('successful delegation', () {
      setUp(() {
        registerFallbackValue(
          const ManualSeedOverrideParams(participants: [], constraints: []),
        );
      });
      ...
      test('constructs constraints correctly and calls service', () async {
        ...
        when(
          () => mockService.reseedAroundPins(any()),
      ...
      test(
        'both dojang and regional enabled constructs 2 constraints in order',
        () async {
          const resultObj = SeedingResult(
            placements: [],
            appliedConstraints: [],
          ...
          expect(
            paramsInCall.constraints[0],
            isA<DojangSeparationConstraint>(),
          );
          expect(
            paramsInCall.constraints[1],
            isA<RegionalSeparationConstraint>(),
          );
        },
      );

      test('SeedingFailure propagated from service', () async {
        when(() => mockService.reseedAroundPins(any())).thenReturn(
