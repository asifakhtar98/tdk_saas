...
void main() {
  ...
  group('ApplyByeAssignmentUseCase', () {
    group('Validation (no service calls)', () {
      ...
      test('< 2 participants → ValidationFailure', () async {
        final result = await useCase(
          const ApplyByeAssignmentParams(
            divisionId: 'div1',
            participants: [SeedingParticipant(id: 'p1', dojangName: 'Tiger')],
          ),
        );
        expect(result.isLeft(), isTrue);
        verifyNever(() => mockService.assignByes(any()));
    ...
    group('Delegation', () {
      test(
        'valid params → delegates to service with correct ByeAssignmentParams',
        () async {
          ...
          final captured =
              verify(() => mockService.assignByes(captureAny())).captured.single
                  as ByeAssignmentParams;
