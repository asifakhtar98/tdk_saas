...
void main() {
  ...
  late StreamController<Either<Failure, UserEntity?>> authStateController;
  ...
  group('AuthenticationBloc', () {
    ...
    group('AuthenticationCheckRequested', () {
      blocTest<AuthenticationBloc, AuthenticationState>(
        ...
        act: (bloc) => bloc.add(const AuthenticationEvent.checkRequested()),
        expect: () => [
    ...
    group('AuthenticationUserChanged', () {
      blocTest<AuthenticationBloc, AuthenticationState>(
        ...
        build: buildBloc,
        act: (bloc) => bloc.add(AuthenticationEvent.userChanged(testUser)),
      ...
      blocTest<AuthenticationBloc, AuthenticationState>(
        ...
        build: buildBloc,
        act: (bloc) => bloc.add(const AuthenticationEvent.userChanged(null)),
    ...
    group('AuthenticationSignOutRequested', () {
      blocTest<AuthenticationBloc, AuthenticationState>(
        ...
        act: (bloc) => bloc.add(const AuthenticationEvent.signOutRequested()),
        expect: () => [
          const AuthenticationState.signOutInProgress(),
          const AuthenticationState.unauthenticated(),
        ],
      );
      ...
      blocTest<AuthenticationBloc, AuthenticationState>(
        ...
        setUp: () {
          when(() => mockSignOutUseCase(any())).thenAnswer(
            (_) async => const Left(SignOutFailure(technicalDetails: 'Failed')),
          );
        },
        act: (bloc) => bloc.add(const AuthenticationEvent.signOutRequested()),
        expect: () => [
    ...
    group('auth state stream subscription', () {
      blocTest<AuthenticationBloc, AuthenticationState>(
        ...
        build: buildBloc,
        act: (bloc) async {
          authStateController.add(Right<Failure, UserEntity?>(testUser));
      ...
      blocTest<AuthenticationBloc, AuthenticationState>(
        ...
        build: buildBloc,
        act: (bloc) async {
          authStateController.add(const Right<Failure, UserEntity?>(null));
          await Future<void>.delayed(const Duration(milliseconds: 100));
        },
        expect: () => [const AuthenticationState.unauthenticated()],
      );
        ...
        act: (bloc) async {
          authStateController.add(
            const Left<Failure, UserEntity?>(
              ServerConnectionFailure(technicalDetails: 'Stream error'),
            ),
          );
          await Future<void>.delayed(const Duration(milliseconds: 100));
        },
    ...
    test('subscribes to authStateChanges during '
        'construction', () {
      buildBloc();
