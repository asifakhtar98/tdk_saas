...
void main() {
  ...
  setUp(() {
    ...
    datasource = OrganizationLocalDatasourceImplementation(mockDatabase);
  });
  ...
  group('OrganizationLocalDatasource', () {
    group('getOrganizationById', () {
      test('returns OrganizationModel when found', () async {
        when(
          () => mockDatabase.getOrganizationById('org-1'),
        ).thenAnswer((_) async => testEntry);

        final result = await datasource.getOrganizationById('org-1');

        expect(result, isNotNull);
        expect(result!.id, 'org-1');
        expect(result.name, 'Dragon Martial Arts');
        verify(() => mockDatabase.getOrganizationById('org-1')).called(1);
      });

      test('returns null when not found', () async {
        when(
          () => mockDatabase.getOrganizationById('nonexistent'),
        ).thenAnswer((_) async => null);

        final result = await datasource.getOrganizationById('nonexistent');
    ...
    group('getOrganizationBySlug', () {
      test('returns OrganizationModel when found', () async {
        when(
          () => mockDatabase.getOrganizationBySlug('dragon-martial-arts'),
        ).thenAnswer((_) async => testEntry);

        final result = await datasource.getOrganizationBySlug(
          'dragon-martial-arts',
        );
      ...
      test('returns null when not found', () async {
        ...
        final result = await datasource.getOrganizationBySlug('nonexistent');
    ...
    group('getActiveOrganizations', () {
      test('returns list of OrganizationModels', () async {
        when(
          () => mockDatabase.getActiveOrganizations(),
        ).thenAnswer((_) async => [testEntry]);
    ...
    group('insertOrganization', () {
      test('calls database insertOrganization', () async {
        ...
        when(
          () => mockDatabase.insertOrganization(any()),
        ).thenAnswer((_) async => 1);

        await datasource.insertOrganization(model);

        verify(() => mockDatabase.insertOrganization(any())).called(1);
      });
    });
    ...
    group('updateOrganization', () {
      test('calls database updateOrganization', () async {
        final model = OrganizationModel.fromDriftEntry(testEntry);
        when(
          () => mockDatabase.updateOrganization(any(), any()),
        ).thenAnswer((_) async => true);

        await datasource.updateOrganization(model);

        verify(() => mockDatabase.updateOrganization('org-1', any())).called(1);
      });
    });
    ...
    group('deleteOrganization', () {
      test('calls database softDeleteOrganization', () async {
        ...
        verify(() => mockDatabase.softDeleteOrganization('org-1')).called(1);
      });
    });
  });
}
