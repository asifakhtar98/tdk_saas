...
void main() {
  ...
  group('Validation', () {
    test('empty bracketId → ValidationFailure', () async {
      final result = await useCase(const UnlockBracketParams(bracketId: ''));
  ...
  group('Not finalized', () {
    test(
      'not-finalized bracket → ValidationFailure with "not locked"',
      () async {
        when(
  ...
  group('Success', () {
    ...
    test(
      'unlocks bracket → verifies update called with correct entity',
      () async {
        final bracket = makeBracket(
          isFinalized: true,
          finalizedAtTimestamp: DateTime(2026),
