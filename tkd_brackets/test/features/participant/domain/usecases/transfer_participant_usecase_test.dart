...
void main() {
  ...
  void setupSuccessMocks({
    ...
    when(
      () => mockDivisionRepo.getDivisionById('source-division-id'),
    ).thenAnswer((_) async => Right(sourceDivision ?? tSourceDivision));
    when(
      () => mockDivisionRepo.getDivisionById('target-division-id'),
    ).thenAnswer((_) async => Right(targetDivision ?? tTargetDivision));
    when(
  ...
  group('TransferParticipantUseCase', () {
    group('input validation', () {
      test('returns InputValidationFailure when '
          'participantId is empty', () async {
        ...
        result.fold((failure) {
          ...
          expect(
            (failure as InputValidationFailure).fieldErrors.containsKey(
              'participantId',
            ),
            true,
          );
      ...
      test('returns InputValidationFailure when '
          'targetDivisionId is empty', () async {
        const params = TransferParticipantParams(
    ...
    group('authorization', () {
      test('returns AuthorizationPermissionDeniedFailure '
          'when user has no organization', () async {
        when(
          () => mockUserRepo.getCurrentUser(),
      ...
      test('returns AuthorizationPermissionDeniedFailure '
          'when user org does not match tournament org', () async {
        when(() => mockUserRepo.getCurrentUser()).thenAnswer(
          (_) async => Right(tUser.copyWith(organizationId: 'other-org')),
        );
        when(
          () => mockParticipantRepo.getParticipantById('participant-id'),
        ).thenAnswer((_) async => Right(tParticipant));
        when(
          () => mockDivisionRepo.getDivisionById('source-division-id'),
        ).thenAnswer((_) async => Right(tSourceDivision));
        when(
          () => mockDivisionRepo.getDivisionById('target-division-id'),
        ).thenAnswer((_) async => Right(tTargetDivision));
        when(
          () => mockTournamentRepo.getTournamentById('tournament-id'),
        ).thenAnswer((_) async => Right(tTournament));
    ...
    group('entity not found', () {
      ...
      test('returns NotFoundFailure when '
          'source division not found', () async {
        when(
          () => mockUserRepo.getCurrentUser(),
        ).thenAnswer((_) async => Right(tUser));
        when(
      ...
      test('returns NotFoundFailure when '
          'target division not found', () async {
        when(
          () => mockUserRepo.getCurrentUser(),
        ).thenAnswer((_) async => Right(tUser));
        when(
    ...
    group('transfer guards', () {
      test('returns InputValidationFailure when '
          'participant is already in target division', () async {
        ...
        when(
          () => mockParticipantRepo.getParticipantById('participant-id'),
        ).thenAnswer((_) async => Right(targetParticipant));
        ...
        verifyNever(() => mockDivisionRepo.getDivisionById(any()));
      });

      test('returns InputValidationFailure when '
          'divisions belong to different tournaments', () async {
        setupSuccessMocks(
        ...
        verifyNever(() => mockTournamentRepo.getTournamentById(any()));
      });
    });

    group('division status checks', () {
      test('returns InputValidationFailure when '
          'source division is inProgress', () async {
        setupSuccessMocks(
      ...
      test('returns InputValidationFailure when '
          'source division is completed', () async {
        setupSuccessMocks(
          sourceDivision: tSourceDivision.copyWith(
      ...
      test('returns InputValidationFailure when '
          'target division is inProgress', () async {
        setupSuccessMocks(
          targetDivision: tTargetDivision.copyWith(
            status: DivisionStatus.inProgress,
          ),
      ...
      test('returns InputValidationFailure when '
          'target division is completed', () async {
        setupSuccessMocks(
          targetDivision: tTargetDivision.copyWith(
            status: DivisionStatus.completed,
          ),
      ...
      test('allows transfer when both divisions '
          'are in ready status', () async {
        setupSuccessMocks(
          sourceDivision: tSourceDivision.copyWith(
    ...
    group('successful transfer', () {
      test('transfers participant and resets seedNumber', () async {
        setupSuccessMocks();
        const params = TransferParticipantParams(
          participantId: 'participant-id',
          targetDivisionId: 'target-division-id',
        ...
        final result = await useCase(params);
        expect(result.isRight(), true);
        result.fold((_) => fail('Should be Right'), (updated) {
          expect(updated.divisionId, 'target-division-id');
          expect(updated.seedNumber, isNull);
          expect(updated.syncVersion, tParticipant.syncVersion + 1);
        });
      });
      ...
      test('should call updateParticipant on repository', () async {
        setupSuccessMocks();
        const params = TransferParticipantParams(
          participantId: 'participant-id',
          targetDivisionId: 'target-division-id',
      ...
      test('preserves other participant fields on transfer', () async {
        setupSuccessMocks();
        const params = TransferParticipantParams(
          participantId: 'participant-id',
          targetDivisionId: 'target-division-id',
        ...
        result.fold((_) => fail('Should be Right'), (updated) {
          ...
          expect(updated.firstName, tParticipant.firstName);
          expect(updated.lastName, tParticipant.lastName);
          expect(updated.schoolOrDojangName, tParticipant.schoolOrDojangName);
